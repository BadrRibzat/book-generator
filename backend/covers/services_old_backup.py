...
"""
    ...
    def _vintage_modern_template(self, title1, title2, colors, concept):
        ...
        <head>
            <meta charset="UTF-8">
            <style>
                @import url('https://fonts.googleapis.com/css2?family=Crimson+Text:ital,wght@0,400;0,500;0,600;0,700;0,800;0,900;1,400;1,500;1,600;1,700;1,800;1,900&family=Poppins:wght@300;600&display=swap');
    ...
    def _create_fallback_cover(self, book, concept_index):
        """
        Create a fallback cover when AI generation fails
        Returns a Cover object with a simple design
        """
        from covers.models import Cover
        
        # Generate unique filename
        filename = f"book_{book.id}_fallback_cover_{concept_index}_{random.randint(1000, 9999)}"
        image_path = self.covers_dir / f"{filename}.png"
        pdf_path = self.covers_dir / f"{filename}.pdf"
        
        try:
            # Create simple HTML for fallback cover
            html_content = self._create_fallback_html(book.title)
            ...
            cover = Cover.objects.create(
                book=book,
                template_style=f"fallback_concept_{concept_index}",
                image_path=f"covers/{image_path.name}",
                pdf_path=f"covers/{pdf_path.name}",
                generation_params={
                    'fallback': True,
            ...
            print(f"Fallback cover creation failed: {str(e)}")
            return None
    
    def _create_fallback_html(self, title):
        """Create simple HTML for fallback cover design"""
